...
def _extract_event_rename(message):
    """Return a new event title for explicit event rename requests only."""
    text = (message or "").strip()
    lowered = _normalize_text(text)
    if "task" in lowered or "agenda" in lowered:
        return None

    patterns = [
        r"(?:change|update|edit)\s+(?:the\s+)?event\s+name\s+(?:to|as)\s+(.+)$",
        r"(?:change|update|edit)\s+(?:the\s+)?name\s+of\s+(?:this\s+)?event\s+(?:to|as)\s+(.+)$",
        r"rename\s+(?:this\s+)?event\s+(?:to|as)\s+(.+)$",
        r"(?:call|name)\s+(?:this\s+)?event\s+(.+)$",
    ]
    for pattern in patterns:
...
def _is_event_delete_request(message):
    """True only for deleting/canceling the active event, not tasks or agenda items."""
    lowered = _normalize_text(message)
    if "task" in lowered or "agenda" in lowered or "timeline" in lowered or "lineup" in lowered:
        return False
    return any(w in lowered for w in ["delete", "remove", "cancel"]) and any(
        p in lowered for p in ["this event", "the event", "current event", "delete event", "remove event", "cancel event"]
    )


def _delete_event_for_user(user_id, event_id):
    db = get_db()
    event = db.execute("SELECT * FROM events WHERE id = ? AND user_id = ?", (event_id, user_id)).fetchone()
    if not event:
        return None, "I couldn’t find that event."

    # Clear dependent planning records first so SQLite foreign keys do not block deletion.
    agenda_rows = db.execute("SELECT id FROM agenda_items WHERE event_id = ?", (event_id,)).fetchall()
...
def chat():
    ...
    try:
        ...
        if event_rename_title and target_for_planning:
            old_title = target_for_planning.get("title") or "this event"
            ...
            _save_chat_state(chat_state)
            return jsonify({
                "reply": f"Renamed the event from '{old_title}' to '{event_rename_title}'.",
                "action": "event_renamed",
                "event": refreshed,
                "event_id": target_for_planning["id"]
            }), 200

        if _is_event_delete_request(user_message) and target_for_planning:
            deleted, error = _delete_event_for_user(user_id, target_for_planning["id"])
            if deleted:
                chat_state["last_event_id"] = None
                chat_state["pending_followup"] = None
                _save_chat_state(chat_state)
                return jsonify({
                    "reply": f"Deleted the event '{deleted['title']}'.",
                    "action": "event_deleted",
                    "event": deleted,
                    "event_id": deleted["id"]
                }), 200
            return jsonify({"reply": error or "I couldn’t delete that event.", "action": "event_delete_failed"}), 200
